...
export function WhoIsWatching({ onProfileSelect }: Props) {
  ...
  const handleGridLayout = (event: LayoutChangeEvent) => {
    ...
    profiles.forEach((profile, index) => {
      const columnCount = 3; // Adjust based on your grid
      const itemWidth = width / columnCount;
      const x = (index % columnCount) * itemWidth;
      const y = Math.floor(index / columnCount) * 120; // Assuming height is 120
      const imageSize = itemWidth * 0.8;

      setProfileLayouts((prev) => ({
        ...prev,
        [profile.id]: {
          x: x + (itemWidth - imageSize) / 2,
          y: y + (120 - imageSize) / 2,
          width: imageSize,
          height: imageSize,
          originalX: x,
  ...
  return (
    ...
    >
      ...
      {profiles.map((profile, index) => {
        const layout = profileLayouts[profile.id];
        console.log("laoyout", selectedId);
